...
export default function TestCreation({ editTestId }) {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(!!editTestId);
  ...
  useEffect(() => {
    if (editTestId) {
      const fetchTest = async () => {
        try {
          const res = await apiFetch(`http://localhost:5204/tests/${editTestId}`);
  ...
  const saveTest = async () => {
    ...
    try {
      let res;
      if (editTestId) {
        res = await apiFetch(`http://localhost:5204/tests/${editTestId}`, {
          method: 'PUT',
          body: JSON.stringify(testData),
        });
      } else {
        res = await apiFetch('http://localhost:5204/tests', {
          method: 'POST',
          body: JSON.stringify(testData),
        });
      }
      if (!res.ok) throw new Error('Failed to save test');
      alert(editTestId ? 'Тест обновлён!' : 'Тест сохранён!');
      navigate('/profile');
    } catch (err) {
  ...
  const submitVerify = () => {
    ...
    const results = test.questions.map((q, idx) => {
      ...
      return { userAns, correct, isCorrect };
    });
    ...
    <main className="container my-5">
      <div className="text-center mb-5 p-4 bg-white rounded-4 shadow-sm mx-auto" style={{ maxWidth: '800px' }}>
        <h1 className="display-5 fw-bold">
          {editTestId ? 'Редактировать тест' : 'Создавай, проверяй и показывай ответы мгновенно'}
        </h1>
        <p className="lead text-muted mt-3">
          {editTestId ? 'Измените вопросы, ответы или название теста.' : 'GradeTest — удобный инструмент для преподавателей и студентов. Легко создавайте тесты, проверяйте знания и демонстрируйте правильные ответы за секунды.'}
        </p>
        <div className="mt-3">
          ...
          <div className="card border-0 shadow-sm h-100">
            <div className="card-body p-4">
              <div className="d-flex align-items-center gap-2 mb-3">
                ...
                <h2 className="h4 mb-0 fw-bold">{editTestId ? 'Редактировать' : 'Создать тест'}</h2>
              </div>
              ...
              <div className="d-flex flex-wrap gap-2">
                ...
                <button className="btn linear-gradient-flame" onClick={saveTest}>
                  <i className="bi bi-save"></i> {editTestId ? 'Обновить тест' : 'Сохранить тест'}
                </button>
            ...
            <div className="card-body p-4">
              ...
              {verifyResult && (
                <div id="verifyResultArea" className="mt-3 alert alert-info">
                  <p><strong>Результат:</strong> {verifyResult.correctCount} из {verifyResult.total} правильных</p>
                  {verifyResult.results.map((r, idx) => (
                    <div key={idx} className={r.isCorrect ? 'text-success' : 'text-danger'}>
                      Вопрос {idx + 1}: {r.isCorrect ? '✓' : '✗'}
                      {!r.isCorrect && (
                        <span className="small ms-2">
                          (Ваш ответ: {r.userAns.join(', ') || 'не выбран'}, правильный: {r.correct.join(', ')})
                        </span>
                      )}
                    </div>
                  ))}
